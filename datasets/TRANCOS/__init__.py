...
class Trancos(BaseLoader):
    def __init__(self, data_dir, transforms, crop_size=400, scale=8, mode='train'):
        super(Trancos, self).__init__(data_dir, transforms, crop_size, scale, mode)
    ...
    def get_file_path(self):
        ...
        img_path_list = sorted(img_path_list, key=lambda s: int(osp.basename(s)[-8:-4]))
        img_path_list = sorted(img_path_list, key=lambda s: int(osp.basename(s)[6]))
