...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Script for preprocess dataset UAVVC')
...
def main():
    ...
    if dst_data_root is None:
        ...
        dst_data_root = osp.join(project_path, 'processed_data/UAVVC')
    else:
    ...
    for mode in ['train', 'test']:
        ...
        src_img_path_list = glob.glob(osp.join(src_data_root, mode, 'image', '*.jpg'))

        if len(src_img_path_list) == 0:
            print('Error: no images found in ', src_data_root)
            return
        else:
            src_img_path_list = sorted(
                src_img_path_list, key=lambda s: int(s[-10:-4]))
            src_img_path_list = sorted(
                src_img_path_list, key=lambda s: int(osp.basename(s).split('_')[0]))

        for i, src_img_path in enumerate(src_img_path_list):
            src_img_name = osp.basename(src_img_path)
            frame = int(src_img_name.split('_')[0])
            sequence = int(src_img_name[-10:-4])
            src_ann_path = src_img_path.replace(
                'image', 'anno').replace('.jpg', '.mat')
            ...
            for point in gt:
                ...
                if 0 <= x < src_w and 0 <= y < src_h:
                    x = min(int(x / rate_w), dst_w)
                    y = min(int(y / rate_h), dst_h)
                    dot_map[y, x] = 1
